...
# Function to fetch YouTube video title using yt-dlp
def fetch_yt_video(link):
    try:
        # Initialize yt-dlp and extract video information
        ydl_opts = {
        ...
        # Handle download error and display message
    ...
    if choice == "Admin":
        ...
        if st.button('Login'):
            if ad_user == 'admin' and ad_password == 'admin':
                ...
                fig = px.histogram(df, x='Resume Score', nbins=20, title="Resume Score Distribution")
                st.plotly_chart(fig)

                # Resume Score vs Experience Level
                st.subheader("Resume Score vs User Level")
                fig = px.box(df, x='User Level', y='Resume Score', title="Resume Score vs User Level")
                st.plotly_chart(fig)
